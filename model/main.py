...
TRPath = './Data/Houston/Houston_eachClass_40_train_40.mat'
TSPath = './Data/Houston/Houston_eachClass_40_test_40.mat'
...
# # #
cnn = LRDTN(classes=class_number, HSI_Data_Shape_H=m, HSI_Data_Shape_W=n, HSI_Data_Shape_C=l,
...
for epoch in range(EPOCH):
    ...
    for step, (images, points, labels) in enumerate(
        ...
        features3, output,T_Net_feature = cnn(images,images)#dsj返回两个数
        features_Tnet.append(T_Net_feature)
        labels_Tnet.append(labels)
        # 分类损失
        ...
        if step % 50 == 0:  # 迭代50次，测试
            ...
            for i in range(number):
                ...
                # temp_points = temp[:, :, pad_width, pad_width]#传递像素的
                temp_points = temp  # 传递四位大小
                temp = temp.cuda()
                temp_points = temp_points.cuda()
                _, temp2,feature_low = cnn(temp, temp_points)
            ...
            if accuracy > BestAcc:
                torch.save(cnn.state_dict(), 'HU2013.pkl')
                BestAcc = accuracy
            cnn.train()

    # with open('./T_Net/features_Tnet', 'wb') as f:
    #     pickle.dump(features_Tnet, f)
    # with open('./T_Net/labels_Tnet', 'wb') as f:
    #     pickle.dump(labels_Tnet, f)

# end training time
end_training_time = datetime.datetime.now()

# # test each class accuracy
cnn.load_state_dict(torch.load('HU2013.pkl'))
cnn.eval()
