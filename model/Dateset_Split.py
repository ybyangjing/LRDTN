...
import h5py
def split_dataset_equal(gt_2D, numTrain=40):
    """
    功能：按照每类数量分割训练集与测试集
    输入：（二维原始标签y，每类训练数量）
    输出：训练集一维坐标,测试集一维坐标
...
GTDataPath = 'D:/GMA-daima/GMA_Net - change/Data/PaviaU/PaviaU_gt.mat'
GT = io.loadmat(GTDataPath)
GT = GT['paviaU_gt']#PU数据集
train_gt_2D, test_gt_2D = split_dataset_equal(GT, numTrain=40)
io.savemat('./Data/PaviaU/PaviaU_eachClass_40_train_40.mat', {'data': train_gt_2D})  #保存mat文件
io.savemat('./Data/PaviaU/PaviaU_eachClass_40_test_40.mat', {'data': test_gt_2D})